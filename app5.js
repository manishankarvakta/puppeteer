...
async function run(slipPaidId, gOrder, noSlip, numChunk) {
    // slip id array
dotenv.config();
const slipLists =  genrateSlipId(slipPaidId, gOrder, noSlip); //'92410202128486454'
// console.log(chunk(slipLists));
const slipChunk = chunk(slipLists, numChunk);
...
// Loops and iteration
function chunk(slipList, numChunk){
  const slipLists = []
    var chunk;

  while (slipList.length > 0) {

    chunk = slipList.splice(0,numChunk)
...
// start app
run('92410202128486454', 'DSC', '10', '5');    
